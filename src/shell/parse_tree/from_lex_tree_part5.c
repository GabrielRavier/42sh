    ...
    struct shell_parse_tree *right)
{
    ...
    result->type = PARSE_TREE_TYPE_AND;
    result->node_left = left;
    result->node_right = right;
    return result;
}

struct shell_parse_tree *shell_parse_tree_from_lex_tree_part5(
    ...
    for (struct lexical_word_list *i = word_list; i != word_list_end;
        ...
        case '&':
            if (i->word[1] != '&' || parens != 0)
                continue;
            return make_result(shell_parse_tree_from_lex_tree_part6(self,
                word_list, i, flags), shell_parse_tree_from_lex_tree_part5(self,
                i->next, word_list_end, flags));
        }
    return shell_parse_tree_from_lex_tree_part6(self, word_list, word_list_end,
        flags);
}
