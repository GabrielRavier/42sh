    ...
    int status)
{
    if (WIFSTOPPED(status)) {
        i->flags |= SH_PROC_FLAG_STOPPED;
        i->reason = WSTOPSIG(status);
    } else {
        if (WIFSIGNALED(status)) {
            if (WTERMSIG(status) == SIGINT)
                i->flags |= SH_PROC_FLAG_INTERRUPTED;
            else
                i->flags |= SH_PROC_FLAG_SIGNALED;
            if (WCOREDUMP(status))
                i->flags |= SH_PROC_FLAG_DUMPED_CORE;
            i->reason = WTERMSIG(status);
        } else {
            i->reason = WEXITSTATUS(status);
            i->flags |= (i->reason != 0) ? SH_PROC_FLAG_ABNORMAL_EXIT :
                SH_PROC_FLAG_NORMAL_EXIT;
        }
    }
}
    ...
    struct shell_proc *proc, int job_flags)
{
    ...
    if ((job_flags & SH_PROC_FLAG_FOREGROUND) && (!(job_flags & (
        SH_PROC_FLAG_SIGNALED | SH_PROC_FLAG_STOPPED) || i->current_dir ==
        NULL || shell_char_strcmp(self->current_dir->name, i->current_dir->name)
        != 0)) && (job_flags & SH_PROC_FLAG_INTERRUPTED))
        shell_putchar(self, '\n');
}
    ...
    int status)
{
    ...
    proc->flags &= ~(SH_PROC_FLAG_RUNNING | SH_PROC_FLAG_STOPPED |
        SH_PROC_FLAG_REPORTED);
    do_next_wait_found_set_status_reason(proc, status);
    job_flags = make_job_flags(proc);
    proc->flags &= ~SH_PROC_FLAG_FOREGROUND;
    if ((job_flags & (SH_PROC_FLAG_RUNNING | SH_PROC_FLAG_REPORTED)) == 0)
